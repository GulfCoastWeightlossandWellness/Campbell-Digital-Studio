...
export const projects: Project[] = [
  ...
  // ── Interactive Health Education ──────────────────────────────────────────
  {
    ...
    features: [
      // ── Marketing Site (interactivehealtheducation.com) ──────────────────
      ...
      // ── Dashboard (dashboard.interactivehealtheducation.com) ─────────────
      ...
      "Shared clinical component library: PhysicianDisclaimer, SourcesAndClinicalReferences, DisclaimerModal, FloatingNavigation, SmoothSlider, AppHeader",
      "Drug interactions database (drug-interactions-data.ts) powering a multi-drug checker with severity ratings",
      "Clinical calculators embedded across modules: Wells DVT/PE, PERC rule, eGFR/CKD-EPI 2021, Framingham 10-yr risk, ABI staging, IPSS urinary score, AUDIT-C, pack-year, Ganzoni iron deficit, Child-Pugh/MELD, BMI/excess weight loss",
      "Interactive tools across modules: symptom sliders, plate builders, breathing timers, body diagrams, exposure ladders, habit trackers, Epley/Dix-Hallpike protocol walkthroughs, mood scales",
      "No PHI collection — fully front-end delivery model safe for clinic embedding without HIPAA agreements",
    ],
    valuePoints: [
      ...
      "No PHI means zero compliance overhead for clinic partners — just a link or embed",
      "Policy engines with unit tests — the catalog logic is tested, not just built",
      "Tiered pricing from $1K/year (small clinic) to enterprise custom — designed for real B2B sales",
      "The marketing site speaks to specific buyer personas with dedicated landing pages and an ROI calculator",
      "Physician-authored content — the clinical accuracy is part of the product value, not just a disclaimer",
    ],
    stack:
    ...
    whatIBuilt:
      "Two fully distinct surfaces built for different audiences. The marketing site targets clinic buyers, health systems, and digital health companies — with buyer-persona SEO landing pages, solution verticals for behavioral health, employer health, and weight management, a tiered pricing page, a ROI calculator, a live demo flow, and an admin management layer. The dashboard is the product itself: a React SPA with 145 physician-authored interactive patient education apps, each lazy-loaded and fully functional as a standalone module. The catalog is governed by a policy engine architecture — commercial bundles and curated collections are defined in source JSON and auto-generated into TypeScript modules, meaning the catalog structure is version-controlled, testable, and independent from the UI. The demo mode gates specific apps for prospective buyers and uses a smart presentation order to showcase the platform's range during sales calls.",
    seoConversion:
      "The marketing site is structured around B2B buyer-persona discovery: 10+ dedicated landing pages targeting specific clinical use cases (diagnosis-linked education, post-discharge, care gap closure, medication adherence, mental health, health systems, digital health companies, white-label). Each page targets a distinct buyer context and search intent. The ROI calculator gives buyers a quantified justification for the purchase. The live demo reduces the friction of a sales call by letting buyers self-explore. Solution vertical pages address the four highest-value clinical markets with dedicated copy and proof points.",
    businessValue:
      "Interactive Health Education demonstrates product-level thinking that goes far beyond website work. The marketing site handles discovery and buyer conversion. The dashboard is the actual deliverable — a physician-designed library of 145 interactive patient education tools organized into 11 commercial licensing bundles, with a policy engine that governs access, a demo mode built for sales, and a no-PHI architecture that removes the compliance barrier that stops most clinic software deals. The platform supports four business models simultaneously: direct clinic licensing, health system enterprise contracts, digital health SaaS partnerships, and white-label distribution. It is a fully operational digital health product.",
    screenshotLabels: [
